...
def MailSender(filename,time):
    try:
        fromaddr = "knagane12792@gmail.com"
        ...
        msg.attach(MIMEText(body, 'plain'))  #attach mail body
    
        attachment = open(filename, "rb") #attachment attach to email file
    
        p = MIMEBase('application', 'octet-stream')
    
        p.set_payload((attachment).read()) #
    
        encoders.encode_base64(p) #encode to decode algorithm for data security
        ...
        s.login(fromaddr,"jbkd vqqa uhrs tgsx")
...
def ProcessLog(log_dir ='Marvellous'):
    ...
    separator = "-" * 80
    log_path = os.path.join(log_dir,"MarvellousLogML%s.log"%(time.ctime()))
    f = open(log_path, 'w')
    f.write(separator + "\n")
...
#to run python MailSchedule.py 1 ....1 is minutes that wants to create logfile and send through emial